...
export default function Register() {
  ...
  const handleSubmit = () => {
    ...
    if (noOfEmptyFields.length >1) {
        ...
        setLoading(false);
        showMessage("Please provide valid username", NotificationTypeEnum.Error);
        return;
    }
    else if (emailError || !email){
        setLoading(false);
        showMessage("Please provide valid email", NotificationTypeEnum.Error);
        return;
    }
    else if (dobError || !dob){
        setLoading(false);
        showMessage("Please provide valid date of birth", NotificationTypeEnum.Error);
        return;
    }

    RequestHelper.Post('register', {
        email: email,
        username: username,
        birthdate: dob.$d,
      }, (res, success)=>{
        ...
        >
          ...
          <Box>
            ...
            <LocalizationProvider dateAdapter={AdapterDayjs}>
              <DatePicker className="register-datepicker form-input" 
